...
instance Show RegionAST where
    show = showHelper 0
      where
        showHelper level (R reg) = case reg of
          (FromPoints pts) -> indent level ++ "FromPoints " ++ show pts
          (Rectangle w h) -> indent level ++ "Rectangle " ++ show w ++ " " ++ show h
          (Circle r) -> indent level ++ "Circle " ++ show r
          (Complement r) -> indent level ++ "~\n" ++ showHelper (level + 2) r
          (Union r1 r2) -> indent level ++ "+\n" ++ showHelper (level + 2) r1 ++ "\n" ++ showHelper (level + 2) r2
          (Intersection r1 r2) -> indent level ++ "*\n" ++ showHelper (level + 2) r1 ++ "\n" ++ showHelper (level + 2) r2
          (Transform t r) -> indent level ++ show t ++ "\n" ++ showHelper (level + 2) r
...
instance Num RegionAST where
    ...
    -}
    region1 - region2 = intersection region1 (negate region2)
...
toTransformation = foldTransformationAST combiner
  where
    ...
    combiner transformation = case transformation of
      ...
      (Combine trans) -> S.combineTransformations trans
...
{-
    *** TODO ***
    ...
    Răspuns: intarzie evaluarea celei de-a doua componente pana cand avem nev de
      ea
...
basicEntityCount = foldRegionAST combiner
  where
    ...
    combiner region = case region of
      ...
      (Circle _) -> (1, 0)
      (Complement reg) -> reg
      (Union (reg1, transCount1) (reg2, transCount2)) -> (reg1 + reg2, transCount1 + transCount2)
      (Intersection (reg1, transCount1) (reg2, transCount2)) -> (reg1 + reg2, transCount1 + transCount2)
      (Transform trans (reg, transCount)) -> (reg, transCount + basicTransformationCount trans)
