...
{-
    *** TODO ***
    ...
    fromPoints points point = point `elem` points
...
promoteBinary f x y = \point -> f (x point) (y point)
...
{-
    *** TODO ***
    ...
    applyTransformation trans region = \point -> region (trans point) 
-}
...
{-
    *** TODO ***
    ...
    combineTransformations transformations = \id ->
    foldl (\acc trans -> trans acc) id transformations
-}
...
{-
    *** TODO ***
    ...
    Răspuns: rezultatul transformatii nu este calculat imediat, ci doar cand
        avem nevoie (la regiunea finala). Daca pct nu apartine, ev lenesa
        amana calculul altor regiuni intermediare
-}
circles :: Int -> Region
circles n
    | n <= 0    = const False
    | otherwise = union (circle 2)
                        (applyTransformation (translation 6 0) (circles (n - 1)))
...
{-
    *** TODO ***
    ...
    Răspuns: se verifica apartenenta la nesfarsit. lazy evaluation ->
        se face urm verificare doar cand verif anterioara e falsa, dar din
        cauza ca punctul nu apartine regiunii va merge pana la infinit
-}
infiniteCircles :: Region
infiniteCircles = union (circle 2)
                        (applyTransformation (translation 6 0) infiniteCircles)
