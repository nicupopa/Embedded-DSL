...
toRegion region = case region of
    ...
    (Union r1 r2) -> S.union (toRegion r1) (toRegion r2)
    (Intersection r1 r2) -> S.intersection (toRegion r1) (toRegion r2)
    (Transform transformation region) -> 
        S.applyTransformation (toTransformation transformation) (toRegion region)
...
fuseTransformations = foldr fuse []
  where
    fuse (Translation tx1 ty1) (Translation tx2 ty2 : rest) = Translation (tx1 + tx2) (ty1 + ty2) : rest
    fuse (Scaling factor1) (Scaling factor2 : rest) = Scaling (factor1 * factor2) : rest
    fuse transformation rest = transformation : rest
